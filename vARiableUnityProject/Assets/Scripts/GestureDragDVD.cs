...
public class GestureDragDVD : MonoBehaviour {
    Vector3 deltaPos;
    ...
    void OnTapHoldAndDrag(Vector3 mouse_velocity)
    {
        ...
        if (transform.localPosition.x +
                Vector3.Dot(mouse_move_dis, this.transform.right.normalized) / 5 > -0.148 &&

                transform.localPosition.x +
                Vector3.Dot(mouse_move_dis, this.transform.right.normalized) / 5 <= -0.0782)
        {
